...
// Tworzenie kolejki priorytetowej na podstawie tablicy.
PriorityQueue buildPriorityQueue(int freq[]){
    ...
    //  Wykonujemy heapify dla ca³ej kolejki
...
Node* buildHuffman(int freq[]){
    PriorityQueue pq = buildPriorityQueue(freq); //Tworzymy kolejkê na podstawie tablicy
...
void compressText(char* input, char* output){
    ...
    // Zapis tekstu w postaci bitowej.
    BitWriter bw; initBitWriter(&bw,out);
    for(int i=0;i<len;i++){
...
void decompressText(char* input, char* output){
    ...
    // Odczyt tekstu w trybie binarnym — dok³adnie taka liczba znaków, jaka wystêpowa³a w oryginale.
    BitReader br; initBitReader(&br,in);
    Node* curr=root;
    int bit;
    int decoded = 0;
